"""
5-Dimensional Meta-Knowledge Graph Agent

Builds comprehensive knowledge graph from multiple sources:
- Custom user schema
...
Classifies into 5 dimensions:
1. UNIVERSAL - Timeless facts (people, companies, concepts)
2. TEMPORAL - Time-based events (meetings, transactions, milestones)
3. SPATIAL - Location-based (offices, travel, geographic data)
4. NARRATIVE - Stories and sequences (project timelines, email threads)
5. CUSTOM - User-defined categories
"""
...
class KnowledgeAgent(BaseAgent):
    ...
    Aggregates knowledge from all sources and classifies into dimensions
    """
    
    def __init__(self):
        super().__init__(name="KnowledgeAgent", model="phi-3-medium")
        self.llm_url = "http://localhost:8080"
        self.client = httpx.AsyncClient(timeout=60.0)
        self.atlas_api_url = os.getenv("ATLAS_API_URL", "http://atlas:8000")
    ...
    def get_static_knowledge(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": ['knowledge search', 'semantic search', 'Q&A']
        }
    ...
    def _should_use_llm(self, data: Dict[str, Any]) -> bool:
        ...
        return any(key in data for key in ['query', 'question', 'search'])
    ...
    async def analyze_with_llm(self, data: Dict[str, Any], static_knowledge: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt = f"""You are a knowledge base management expert. Analyze this specific situation:
...
2. What are the relevant sources?
3. What is the confidence level?
4. What related information exists?
